...
| 方法 | Test Accuracy | Test F1-macro | 训练可训练参数量 | 峰值显存 (VRAM) |
|------|:---:|:---:|:---:|:---:|
| **Full Fine-tuning** | 94.5% ~ 96.0% | 0.945 ~ 0.960 | ~102M (100%) | ~8.0 GB |
| **LoRA (r=8)** | 93.0% ~ 95.0% | 0.930 ~ 0.950 | ~1.1M (1.07%) | ~4.2 GB |

> **实验结论**：LoRA 微调方法仅需调整 **1.07%** 的可训练参数，便可逼近全量微调 **98%** 以上的性能，同时将显存占用降低近一倍（~4.2GB），极大提升了消费级显卡上的微调效率。
